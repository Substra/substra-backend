    ...
    def on_failure(
        ...
        with get_orchestrator_client(channel_name) as client:
            client.update_function_status(
                {"key": function_key, "status": orchestrator.function_pb2.FUNCTION_STATUS_FAILED}
            )
...
class SaveImageTask(BaseBuilderTask):
    ...
    def on_success(self, retval: dict[str, Any], task_id: str, args: tuple, kwargs: dict[str, Any]) -> None:
        ...
        with get_orchestrator_client(channel_name) as client:
            client.update_function_status(
                {"key": function_key, "status": orchestrator.function_pb2.FUNCTION_STATUS_READY}
            )
...
class BuildTask(BaseBuilderTask):
    ...
    def before_start(self, task_id, args, kwargs):
        ...
        with get_orchestrator_client(channel_name) as client:
            client.update_function_status(
                {"key": function_key, "status": orchestrator.function_pb2.FUNCTION_STATUS_BUILDING}
            )
