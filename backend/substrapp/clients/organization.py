...
def _http_request(
    ...
    response = None
    try:
        logger.debug("Sending HTTP request", method=method, url=url, extra_headers=headers)
        ...
        logger.debug("Finished HTTP request", url=url, status_code=response.status_code)
