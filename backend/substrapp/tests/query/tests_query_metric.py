...
from ..common import get_sample_metric, get_sample_datamanager, \
    AuthenticatedClient, get_sample_metric_metadata
...
class MetricQueryTests(APITestCase):
    ...
    def test_get_metric_metrics(self):
        ...
        with mock.patch('substrapp.views.utils.get_owner', return_value='foo'), \
                mock.patch.object(OrchestratorClient, 'query_metric', return_value=get_sample_metric_metadata()):
            ...
            response = self.client.get(
                f'/metric/{metric.key}/metrics/', **extra)

            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertEqual(self.metric_metrics_filename,
                             response.filename)
