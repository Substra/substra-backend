...
class PrepareTaskTests(APITestCase):
    ...
    def test_task_is_not_runnable(self):
        ...
        with mock.patch.object(OrchestratorClient, "query_task", return_value=doing_task), \
                mock.patch(
                    'celery.app.task.Task.request',
                    new_callable=mock.PropertyMock) as mrequest, \
                mock.patch(
                    'celery.worker.request.Request.delivery_info',
                    new_callable=mock.PropertyMock) as mdeliveryinfo:
    ...
    def test_prepare_tasks_todo_orchestrator_error(self):
        with mock.patch.object(OrchestratorClient, "query_task", return_value=self.task), \
                mock.patch('celery.app.task.Task.request', new_callable=mock.PropertyMock) as mrequest, \
                mock.patch(
                    'celery.worker.request.Request.delivery_info',
                    new_callable=mock.PropertyMock) as mdeliveryinfo, \
                mock.patch.object(OrchestratorClient, "update_task_status") as mtask_status:
            mrequest.return_value = mdeliveryinfo
            mdeliveryinfo.return_value = {"routing_key": "routing_key"}
    ...
    def test_prepare_tasks(self):
        with mock.patch.object(OrchestratorClient, "query_task", return_value=self.task), \
                mock.patch("substrapp.tasks.tasks_prepare_task.compute_task.apply_async",
                           return_value="compute_task") as mapply_async, \
                mock.patch('celery.app.task.Task.request', new_callable=mock.PropertyMock) as mrequest, \
                mock.patch(
                    'celery.worker.request.Request.delivery_info',
                    new_callable=mock.PropertyMock) as mdeliveryinfo, \
