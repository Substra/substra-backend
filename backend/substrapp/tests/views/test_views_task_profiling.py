...
class TaskProfilingViewTests(APITestCase):
    ...
    def setUp(self) -> None:
        ...
        self.url = reverse("substrapp:task_profiling-list")
    ...
    def test_task_profiling_retrieve_success(self):
        response = self.client.get(
            reverse("substrapp:task_profiling-detail", args=[self.expected_results[0]["compute_task_key"]]),
            **self.extra,
        )
...
class TaskProfilingViewTestsBackend(APITestCase):
    ...
    def test_task_profiling_create_success(self):
        ...
        step_url = reverse("substrapp:step-list", args=[str(task.key)])
