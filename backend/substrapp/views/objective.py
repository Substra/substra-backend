...
class ObjectiveViewSet(mixins.CreateModelMixin,
    ...
    def list(self, request, *args, **kwargs):
        try:
            data = query_ledger(request.user.channel.name, fcn='queryObjectives', args=[])
        except LedgerError as e:
            return Response({'message': str(e.msg)}, status=e.status)
    ...
    def leaderboard(self, request, pk):
        ...
        try:
            leaderboard = query_ledger(request.user.channel.name, fcn='queryObjectiveLeaderboard', args={
                'objectiveKey': pk,
