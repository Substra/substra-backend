...
def generate(orgs):
    ...
    for org in orgs:
        ...
        files[org]["outgoing_nodes"].append({"node_id": org, "secret": secret})
        files[org]["incoming_nodes"].append({"node_id": org, "secret": secret})
        ...
        for other_org in filter(lambda x: x != org, orgs):
            ...
            secret = generate_secret()
            files[other_org]["outgoing_nodes"].append({"node_id": org, "secret": secret})  # in server B  # to server A

            files[org]["incoming_nodes"].append(
                {"node_id": other_org, "secret": secret}  # in server A  # from server B
            )
