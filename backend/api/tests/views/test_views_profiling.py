...
def test_task_profiling_retrieve_success(authenticated_client, task_profiling):
    ...
    response = authenticated_client.get(
        reverse("api:task-profiling", args=[expected_results[0]["compute_task_key"]]),
    )
...
def test_task_profiling_create_bad_client(authenticated_client, task_profiling):
    key = str(task_profiling.compute_task.key)
    url = reverse("api:task-profiling", args=[key])
    response = authenticated_client.post(
        url, {"compute_task_key": str(task_profiling.compute_task.key), "channel": CHANNEL}
    )
...
def test_task_profiling_create_success(authenticated_backend_client, create_compute_task):
    ...
    key = str(compute_task.key)
    url = reverse("api:task-profiling", args=[key])
    response = authenticated_backend_client.post(url, {"compute_task_key": str(compute_task.key)}, **EXTRA)
...
def test_already_exist_task_profiling(authenticated_backend_client):
    ...
    key = str(task.key)
    url = reverse("api:task-profiling", args=[key])
    response = authenticated_backend_client.post(url, {"compute_task_key": str(task.key)})
    assert response.status_code == status.HTTP_201_CREATED
...
def test_task_profiling_update_datetime(authenticated_backend_client, create_compute_plan, create_compute_task):
    ...
    authenticated_backend_client.post(url, {"compute_task_key": str(compute_task.key)})
    task_profiling = compute_task.task_profiling
    task_profiling.refresh_from_db()
    previous_datetime = task_profiling.creation_date

    authenticated_backend_client.put(url, {"compute_task_key": str(compute_task.key)}, **EXTRA)
    task_profiling.refresh_from_db()
...
def test_task_profiling_add_step(authenticated_backend_client, task_profiling):
    ...
    assert response.status_code == status.HTTP_201_CREATED
    key = str(compute_task_key)
    expected_results = {
        "compute_task_key": key,
        "execution_rundown": [{"duration": 20000000, "step": "custom_step"}],
        "task_duration": None,
    }

    url = reverse("api:task-profiling", args=[key])
    response = authenticated_backend_client.get(url, **EXTRA)
