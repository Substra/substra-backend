...
class DataManagerViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("localrep:data_manager-list")
    ...
    def test_datamanager_update(self):
        ...
        url = reverse("localrep:data_manager-detail", args=[data_manager["key"]])
    ...
    @mock.patch("localrep.views.datamanager.DataManagerViewSet.list", side_effect=Exception("Unexpected error"))
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.datamanager.DataManagerViewSet.create", side_effect=Exception("Unexpected error"))
    ...
    def test_datamanager_retrieve(self):
        ...
        url = reverse("localrep:data_manager-detail", args=[self.expected_results[0]["key"]])
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.datamanager.DataManagerViewSet.retrieve", side_effect=Exception("Unexpected error"))
    ...
    def test_datamanager_download_opener(self):
        ...
        url = reverse("localrep:data_manager-opener", args=[data_manager.key])
        with mock.patch("localrep.views.utils.get_owner", return_value=data_manager.owner):
            response = self.client.get(url, **self.extra)
    ...
    def test_datamanager_download_description(self):
        data_manager_data = factory.create_datamanager_data()
        data_manager = factory.create_datamanager(key=data_manager_data.key)
        url = reverse("localrep:data_manager-description", args=[data_manager.key])
        with mock.patch("localrep.views.utils.get_owner", return_value=data_manager.owner):
            response = self.client.get(url, **self.extra)
        content = response.getvalue()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
