...
from localrep.views.computetask import EXTRA_DATA_FIELD
...
class TaskBulkCreateViewTests(ComputeTaskViewTests):
    def test_task_bulk_create(self):
        ...
        self.maxDiff = None
        url = reverse("localrep:task_bulk_create")
...
class TrainTaskViewTests(ComputeTaskViewTests):
    def setUp(self):
        super().setUp()
        self.url = reverse("localrep:traintuple-list")
    ...
    @mock.patch("localrep.views.computetask.ComputeTaskViewSet.list", side_effect=Exception("Unexpected error"))
    ...
    def test_traintask_cp_list_success(self):
        ...
        url = reverse("localrep:compute_plan_traintuple-list", args=[self.compute_plan.key])
    ...
    def test_traintask_retrieve(self):
        url = reverse("localrep:traintuple-detail", args=[self.expected_results[0]["key"]])
        response = self.client.get(url, **self.extra)
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.computetask.ComputeTaskViewSet.retrieve", side_effect=Exception("Unexpected error"))
...
class TestTaskViewTests(ComputeTaskViewTests):
    def setUp(self):
        super().setUp()
        self.url = reverse("localrep:testtuple-list")
    ...
    def test_testtask_cp_list_success(self):
        ...
        url = reverse("localrep:compute_plan_testtuple-list", args=[self.compute_plan.key])
        response = self.client.get(url, **self.extra)
        # manually overriding duration for doing tasks as "now" is taken from db and not timezone.now(),
        # couldn't be properly mocked
        for task in response.json().get("results"):
    ...
    def test_testtask_retrieve(self):
        url = reverse("localrep:testtuple-detail", args=[self.expected_results[0]["key"]])
        response = self.client.get(url, **self.extra)
        # patch expected results with extended data
...
class CompositeTaskViewTests(ComputeTaskViewTests):
    def setUp(self):
        super().setUp()
        self.url = reverse("localrep:composite_traintuple-list")
    ...
    def test_compositetask_cp_list_success(self):
        ...
        url = reverse("localrep:compute_plan_composite_traintuple-list", args=[self.compute_plan.key])
        response = self.client.get(url, **self.extra)
        # manually overriding duration for doing tasks as "now" is taken from db and not timezone.now(),
        # couldn't be properly mocked
        for task in response.json().get("results"):
    ...
    def test_compositetask_retrieve(self):
        url = reverse("localrep:composite_traintuple-detail", args=[self.expected_results[0]["key"]])
        response = self.client.get(url, **self.extra)
        # patch expected results with extended data
