...
class DataSampleViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("localrep:data_sample-list")
    ...
    def test_datasample_retrieve(self):
        url = reverse("localrep:data_sample-detail", args=[self.expected_results[0]["key"]])
    ...
    @mock.patch("localrep.views.datasample.DataSampleViewSet.retrieve", side_effect=Exception("Unexpected error"))
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.datasample.DataSampleViewSet.list", side_effect=Exception("Unexpected error"))
    ...
    def test_data_create_upload(self):
        ...
        ), mock.patch("localrep.views.datasample.check_datamanagers", return_value=None):
    ...
    def test_file_size_limit(self):
        ...
        with mock.patch("localrep.views.datasample.check_datamanagers", return_value=None):
            response = self.client.post(self.url, data=data, format="multipart", **self.extra)
    ...
    def test_datasample_create_fail_rollback(self):
        ...
        with mock.patch.object(OrchestratorClient, "register_datasamples", side_effect=MockOrcError()), mock.patch(
            "localrep.views.datasample.check_datamanagers", return_value=None
        ):
            response = self.client.post(self.url, data=data, format="multipart", **self.extra)
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.datasample.DataSampleViewSet.create", side_effect=Exception("Unexpected error"))
    def test_datasample_create_fail(self, _):
        response = self.client.post(self.url, data={}, format="json", **self.extra)
        self.assertEqual(response.status_code, status.HTTP_500_INTERNAL_SERVER_ERROR)

    @internal_server_error_on_exception()
    @mock.patch("localrep.views.datasample.get_channel_name", side_effect=Exception("Unexpected error"))
    def test_datasample_bulk_update_fail(self, get_channel_name: mock.Mock):
        url = django.urls.reverse("localrep:data_sample-bulk-update")
