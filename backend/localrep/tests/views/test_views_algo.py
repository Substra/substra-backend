...
class AlgoViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("localrep:algo-list")
    ...
    @mock.patch("localrep.views.algo.AlgoViewSet.list", side_effect=Exception("Unexpected error"))
    ...
    def test_algo_cp_list_success(self):
        ...
        url = reverse("localrep:compute_plan_algo-list", args=[compute_plan.key])
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.algo.AlgoViewSet.create", side_effect=Exception("Unexpected error"))
    def test_algo_create_fail(self, _):
        response = self.client.post(self.url, data={}, format="json")
        self.assertEqual(response.status_code, status.HTTP_500_INTERNAL_SERVER_ERROR)

    def test_algo_retrieve(self):
        url = reverse("localrep:algo-detail", args=[self.expected_algos[0]["key"]])
        response = self.client.get(url, **self.extra)
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.algo.AlgoViewSet.retrieve", side_effect=Exception("Unexpected error"))
    ...
    def test_algo_download_file(self):
        ...
        url = reverse("localrep:algo-file", args=[algo.key])
        with mock.patch("localrep.views.utils.get_owner", return_value=algo.owner):
            response = self.client.get(url, **self.extra)
    ...
    def test_algo_download_description(self):
        algo_data = factory.create_algo_data()
        algo = factory.create_algo(key=algo_data.key)
        url = reverse("localrep:algo-description", args=[algo.key])
        with mock.patch("localrep.views.utils.get_owner", return_value=algo.owner):
            response = self.client.get(url, **self.extra)
        content = response.getvalue()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_algo_update(self):
        ...
        url = reverse("localrep:algo-detail", args=[algo["key"]])
        algo["name"] = data["name"]
