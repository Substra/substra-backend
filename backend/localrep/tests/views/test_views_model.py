...
from localrep.views.model import ModelPermissionViewSet
from localrep.views.utils import AssetPermissionError
...
class ModelViewTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("localrep:model-list")
    ...
    @mock.patch("localrep.views.model.ModelViewSet.list", side_effect=Exception("Unexpected error"))
    ...
    def test_model_retrieve(self):
        url = reverse("localrep:model-detail", args=[self.expected_results[0]["key"]])
    ...
    @internal_server_error_on_exception()
    @mock.patch("localrep.views.model.ModelViewSet.retrieve", side_effect=Exception("Unexpected error"))
    ...
    def test_model_download_file(self):
        ...
        url = reverse("localrep:model-file", args=[model.key])
        with mock.patch("localrep.views.utils.get_owner", return_value=model.owner), mock.patch(
            "localrep.views.model.type", return_value=OrganizationUser
        ):
            response = self.client.get(url, **self.extra)
    ...
    def test_model_download_file_wrong_user(self):
        ...
        model = factory.create_model(self.train_task, key=model_data.key, owner="substra")
        url = reverse("localrep:model-file", args=[model.key])
        with mock.patch("localrep.views.utils.get_owner", return_value=model.owner):
            response = self.client.get(url, **self.extra)
