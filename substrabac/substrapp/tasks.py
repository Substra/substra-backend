...
def put_model(subtuple, subtuple_directory, model_content):
    if model_content is not None:
        ...
        try:
            ...
            if get_model_hash(model.file.path, subtuple["model"]["traintupleKey"]) != subtuple['model']['hash']:
                raise Exception('Model Hash in Subtuple is not the same as in local db')

            if not os.path.exists(model_dst_path):
                os.link(model.file.path, model_dst_path)
            else:
                if get_model_hash(model_dst_path, subtuple["model"]["traintupleKey"]) != subtuple['model']['hash']:
    ...
    if models_content:
        ...
        for model_content, subtuple_model in zip(models_content, subtuple['inModels']):
            ...
            try:
                ...
                if get_model_hash(model.file.path, subtuple_model["traintupleKey"]) != subtuple_model['hash']:
                    raise Exception('Model Hash in Subtuple is not the same as in local db')

                if not os.path.exists(model_dst_path):
                    os.link(model.file.path, model_dst_path)
                else:
                    if get_model_hash(model_dst_path, subtuple_model["traintupleKey"]) != subtuple_model['hash']:
                        raise Exception('Model Hash in Subtuple is not the same as in local medias')
...
def doTask(subtuple, tuple_type):
    ...
    try:
        ...
        if tuple_type == 'traintuple':
            ...
            end_model_file_hash = get_model_hash(end_model_path, subtuple['key'])
            try:
