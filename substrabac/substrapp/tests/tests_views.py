...
class ViewTests(APITestCase):
    ...
    def test_challenge_list_empty(self):
        ...
        with mock.patch('substrapp.views.challenge.queryLedger') as mocked_function:
            url = reverse('substrapp:challenge-list')
            mocked_function.return_value = None, status.HTTP_200_OK
        ...
        with mock.patch('substrapp.views.challenge.queryLedger') as mocked_function:
            url = reverse('substrapp:challenge-list')
            mocked_function.return_value = ['ISIC'], status.HTTP_200_OK
    ...
    def test_challenge_list_filter_fail(self):
        ...
        with mock.patch('substrapp.views.challenge.queryLedger') as mocked_function:
            url = reverse('substrapp:challenge-list')
            mocked_function.side_effect = [(challenge, status.HTTP_200_OK), (dataset, status.HTTP_200_OK)]
            search_params = '?search=challen'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
    ...
    def test_challenge_list_filter_metric(self):
        extra = {
            'HTTP_ACCEPT': 'application/json;version=0.0'
        }
    ...
    def test_challenge_list_filter_algo(self):
        ...
        with mock.patch('substrapp.views.challenge.queryLedger') as mocked_function:
            url = reverse('substrapp:challenge-list')
            mocked_function.side_effect = [(challenge, status.HTTP_200_OK), (algo, status.HTTP_200_OK)]
    ...
    def test_challenge_list_filter_model(self):
        ...
        with mock.patch('substrapp.views.challenge.queryLedger') as mocked_function:
            url = reverse('substrapp:challenge-list')
            mocked_function.side_effect = [(challenge, status.HTTP_200_OK), (model, status.HTTP_200_OK)]
    ...
    def test_challenge_retrieve(self):
        ...
        with mock.patch('substrapp.views.challenge.getObjectFromLedger') as mocked_function, \
                mock.patch.object(ChallengeViewSet, 'create_or_update_challenge') as mocked_method:
            mocked_function.return_value = challenge[0]
            mocked_method.return_value = Challenge.objects.create(description=self.challenge_description,
    ...
    def test_challenge_retrieve_fail(self):
        ...
        with mock.patch('substrapp.views.challenge.getObjectFromLedger') as mocked_function:
            mocked_function.side_effect = JsonException('TEST')
            search_params = '6b8d16ac3eae240743428591943fa8e66b34d4a7e0f4eb8e560485c7617c222c/'
            response = self.client.get(url + search_params, **extra)
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_algo_list_empty(self):
        ...
        with mock.patch('substrapp.views.algo.queryLedger') as mocked_function:
    ...
    def test_algo_list_filter_fail(self):
        ...
        with mock.patch('substrapp.views.algo.queryLedger') as mocked_function:
            url = reverse('substrapp:algo-list')
            mocked_function.side_effect = [(algo, status.HTTP_200_OK), (dataset, status.HTTP_200_OK)]
            search_params = '?search=challen'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertIn('Malformed search filters', r['message'])
    ...
    def test_algo_list_filter_challenge(self):
        ...
        with mock.patch('substrapp.views.algo.queryLedger') as mocked_function:
            url = reverse('substrapp:algo-list')
            mocked_function.side_effect = [(algo, status.HTTP_200_OK), (challenge, status.HTTP_200_OK)]
            search_params = '?search=challenge%253Aname%253ASkin%2520Lesion%2520Classification%2520Challenge'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
    ...
    def test_algo_list_filter_model(self):
        ...
        with mock.patch('substrapp.views.algo.queryLedger') as mocked_function:
            url = reverse('substrapp:algo-list')
            mocked_function.side_effect = [(algo, status.HTTP_200_OK), (model, status.HTTP_200_OK)]
            search_params = '?search=model%253Ahash%253A454511615090218bf9cef23b801a517d36045582c43ce7a908acb59b5174f011'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertEqual(len(r[0]), 3)
    ...
    def test_algo_retrieve(self):
        ...
        with mock.patch('substrapp.views.algo.getObjectFromLedger') as mocked_function, \
                mock.patch.object(AlgoViewSet, 'create_or_update_algo') as mocked_method:
            url = reverse('substrapp:algo-list')
            mocked_function.return_value = algo[3]
            mocked_method.return_value = Algo.objects.create(file=self.script)
    ...
    def test_model_list_empty(self):
        ...
        with mock.patch('substrapp.views.model.queryLedger') as mocked_function:
    ...
    def test_model_list_filter_fail(self):
        ...
        with mock.patch('substrapp.views.model.queryLedger') as mocked_function:
            url = reverse('substrapp:model-list')
            mocked_function.side_effect = [(model, status.HTTP_200_OK), (dataset, status.HTTP_200_OK)]
            search_params = '?search=challen'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertIn('Malformed search filters', r['message'])
    ...
    def test_model_list_filter_challenge(self):
        ...
        with mock.patch('substrapp.views.model.queryLedger') as mocked_function:
            url = reverse('substrapp:model-list')
            mocked_function.side_effect = [(model, status.HTTP_200_OK), (challenge, status.HTTP_200_OK)]
            search_params = '?search=challenge%253Aname%253ASkin%2520Lesion%2520Classification%2520Challenge'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertEqual(len(r[0]), 1)
    ...
    def test_model_list_filter_algo(self):
        ...
        with mock.patch('substrapp.views.model.queryLedger') as mocked_function:
            url = reverse('substrapp:model-list')
            mocked_function.side_effect = [(model, status.HTTP_200_OK), (algo, status.HTTP_200_OK)]
            search_params = '?search=algo%253Aname%253ALogistic%2520regression'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertEqual(len(r[0]), 1)

    def test_model_list_filter_model(self):
        extra = {
            'HTTP_ACCEPT': 'application/json;version=0.0'
        }
        ...
        with mock.patch('substrapp.views.model.queryLedger') as mocked_function:
            url = reverse('substrapp:model-list')
            mocked_function.side_effect = [(model, status.HTTP_200_OK), (model, status.HTTP_200_OK)]
            search_params = '?search=model%253Ahash%253A454511615090218bf9cef23b801a517d36045582c43ce7a908acb59b5174f011'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertEqual(len(r[0]), 1)
    ...
    #     with mock.patch('substrapp.views.model.getObjectFromLedger') as mocked_function:
    #         url = reverse('substrapp:model-list')
    #         mocked_function.return_value = model[0]
    ...
    def test_dataset_list_empty(self):
        ...
        with mock.patch('substrapp.views.dataset.queryLedger') as mocked_function:
    ...
    def test_dataset_list_filter_fail(self):
        ...
        with mock.patch('substrapp.views.dataset.queryLedger') as mocked_function:
            url = reverse('substrapp:dataset-list')
            mocked_function.side_effect = [(dataset, status.HTTP_200_OK), (dataset, status.HTTP_200_OK)]
            search_params = '?search=challen'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertIn('Malformed search filters', r['message'])
    ...
    def test_dataset_list_filter_algo(self):
        ...
        with mock.patch('substrapp.views.dataset.queryLedger') as mocked_function:
            url = reverse('substrapp:dataset-list')
            mocked_function.side_effect = [(dataset, status.HTTP_200_OK), (algo, status.HTTP_200_OK)]
    ...
    def test_dataset_list_filter_challenge(self):
        ...
        with mock.patch('substrapp.views.dataset.queryLedger') as mocked_function:
            url = reverse('substrapp:dataset-list')
            mocked_function.side_effect = [(dataset, status.HTTP_200_OK), (challenge, status.HTTP_200_OK)]
            search_params = '?search=challenge%253Aname%253ASkin%2520Lesion%2520Classification%2520Challenge'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertEqual(len(r[0]), 1)
    ...
    def test_dataset_list_filter_model(self):
        ...
        with mock.patch('substrapp.views.dataset.queryLedger') as mocked_function:
            url = reverse('substrapp:dataset-list')
            mocked_function.side_effect = [(dataset, status.HTTP_200_OK), (model, status.HTTP_200_OK)]
            search_params = '?search=model%253Ahash%253A454511615090218bf9cef23b801a517d36045582c43ce7a908acb59b5174f011'
            response = self.client.get(url + search_params, **extra)
            r = response.json()
            self.assertEqual(len(r[0]), 1)
    ...
    def test_dataset_retrieve(self):
        ...
        with mock.patch.object(DatasetViewSet, 'getObjectFromLedger') as mocked_method, \
                mock.patch.object(DatasetViewSet, 'create_or_update_dataset') as mocked_method2:
            url = reverse('substrapp:dataset-list')
            mocked_method.return_value = dataset[1]
            mocked_method2.return_value = Dataset.objects.create(name='slide',
    ...
    def test_dataset_retrieve_fail(self):
        ...
        with mock.patch.object(DatasetViewSet, 'getObjectFromLedger') as mocked_method:
            mocked_method.side_effect = JsonException('TEST')
            url = reverse('substrapp:dataset-list')
            search_params = '6b8d16ac3eae240743428591943fa8e66b34d4a7e0f4eb8e560485c7617c222c/'
            response = self.client.get(url + search_params, **extra)
            self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
