    ...
    stage('Test & Build') {
      parallel {
        stage('Test') {
          ...
          steps {
            sh "apt update"
            sh "apt install gcc curl && mkdir -p /tmp/download && curl -L https://download.docker.com/linux/static/stable/x86_64/docker-18.06.3-ce.tgz | tar -xz -C /tmp/download && mv /tmp/download/docker/docker /usr/local/bin/"
